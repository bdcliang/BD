...
namespace BD.Net
{
    ...
    public class NetInterface
    {
        ...
        /// <summary>
        /// 设置电脑IP，默认网关，子网页码，DNS
        /// </summary>
        /// <param name="ip"></param>
        public static void SetIPAddress(string ip)
        {
            if (!IsIPAddress(ip)) return;
            var gateip = GetGateIP(ip);
            SetIPAddress(new string[] {ip },
                new string[] { "255.255.255.0" },
                new string[] { gateip },
                new string[] { gateip });
        }

        private static string GetGateIP(string ip)
        {
            var arrs = ip.Split('.');
            if (arrs.Length != 4) return "";
            return arrs[0] + "." + arrs[1] + "." + arrs[2] + "." + "1";
        }
        private static bool Validate(string ip)
        {
            var arrs = ip.Split('.');
            if (arrs.Length != 4) return false;
            foreach (var item in arrs)
            {
                int tmp = 0;
                var success= int.TryParse(item,out tmp);
